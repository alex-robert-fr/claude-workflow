...
Le cycle impose des sessions dediees au dev et a la review. Traiter deux tickets en parallele par des changements de branche successifs oblige a remiser du travail en cours, et une remise oubliee finit par etre perdue ou committee au mauvais endroit.

Reussi quand deux tickets avancent en parallele sans qu'aucun n'ait a etre remise.
...
- La creation refuse un emplacement deja pris, et cree la branche si elle n'existe pas encore
...
- L'inventaire vide est annonce comme tel, avec le geste pour en creer un

## Hors scope

- Creer une branche pour elle-meme — la branche d'un cycle est creee a l'ecriture des tests, pas ici
...
Le premier argument route vers l'une des quatre actions, le second porte le nom de branche. La bascule de contexte passe par la capacite native prevue a cet effet, pas par un changement de repertoire — un changement de repertoire ne survivrait pas a l'appel suivant.
...
|---------|--------|----------|--------|---------------------|
| — | — | L'emplacement est calcule, jamais choisi | Un chemin deductible du nom de branche se retrouve depuis n'importe quelle session, sans etat a conserver | Demander le chemin a chaque fois |
...
| — | — | La suppression est confirmee, la creation non | Elle detruit du travail non committe sans recours, alors qu'un emplacement cree en trop se supprime | Confirmer les deux |
