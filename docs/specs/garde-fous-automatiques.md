...
- **Les juges ont une variance réelle** : un même extrait peut être jugé différemment d'un appel à l'autre, comme tout jugement porté par un modèle rapide — c'est la nature d'un garde-fou sémantique, pas un défaut à corriger. Et en jq un verdict `false` se lit avec `if . == false`, jamais `// empty` qui l'avale : le juge disait « défaut » et le hook laissait passer
- **`--safe-mode` désactive aussi les hooks du juge lui-même** : première ligne de défense contre la récursion, `CLAUDE_WORKFLOW_JUDGE_ACTIVE` (hérité par le juge) la seconde — ne jamais retirer l'une sans l'autre
