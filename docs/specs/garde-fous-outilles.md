...
Des scripts deterministes, deployes par `/setup`, qui font respecter les regles du workflow a la place d'instructions adressees au LLM.

## Intention

Une regle ecrite dans un CLAUDE.md ou un skill depend de la bonne volonte du modele : elle est suivie souvent, jamais toujours, et l'ecart ne se voit pas. Un script s'execute a chaque fois, ou echoue de facon visible.

C'est reussi quand aucune regle mecaniquement verifiable du workflow ne repose sur une instruction au LLM.
...
- Une commande aux degats irreversibles est refusee avant execution, et son motif parvient a son demandeur
...
- Un ecart de coherence des specs est signale sans interrompre le cycle
...
- Le jugement de qualite — il appartient au sub-agent de `/pipe-review`, interruptible et qui rend un rapport, pas a un script qui bloque
...
Quatre garde-fous sont declenches par un evenement du harness ; un cinquieme est appele par `/pipe-review`, sans evenement. Les cinq sont copies tels quels par `/setup` : leur seule variabilite passe par les arguments declares dans `settings.json`, eux-memes issus de `workflow-config`.
...
- **Internes** : l'index `docs/specs/README.md`, dont dependent a la fois l'injection de contexte et le controle de coherence
- **Externes** : `jq` ; le formateur et la commande de test du projet, quand il en a
- **Dependants** : `/setup` les deploie et verifie leurs cibles, `/pipe-review` appelle le controle de coherence
...
| `skills/setup/hooks-reference.md` | Role de chaque garde-fou et semantique des codes de sortie |
| `skills/setup/scripts/` | Les cinq scripts, copies tels quels chez le projet |
| `.claude/scripts/check-specs.sh` | Copie deployee du controle de coherence des specs |
