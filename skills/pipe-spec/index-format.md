...
- **Une spec est un fichier a plat dans `docs/specs/`**, jamais un repertoire, jamais prefixee d'un numero. Le nom de fichier est l'identite : il est cite par les autres specs, par les points d'entree et par cet index. Un numero le rend mutable — inserer une feature renommerait la suite et casserait toutes ces references. Le besoin d'organisation visuelle se traite par les sections ci-dessus ; une piece jointe (schema, capture) va a plat dans `docs/specs/assets/`
